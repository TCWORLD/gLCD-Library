...
void gLCD::Init(char Xzero, char Yzero, boolean InvertColour, boolean Phillips, boolean xReverse){
		...
		if (xReverse){ //When xReverse is on, then the value of this parameter is ajusted so that x is mirrored from normal.
			_normalScan = 0x60;
			_inverseScan = 0xC0;
			SendByte(_parameter, _normalScan);      //RGB order, mirror display x or y. 0x68 = RGB, no mirroring
			...
			_normalScan = 0x20;
			_inverseScan = 0x80;
			SendByte(_parameter, _normalScan);      //RGB order, mirror display x or y. 0x28 = RGB, no mirroring
