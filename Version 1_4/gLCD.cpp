/*
	Sparkfun Nokia knockoff screen 128x128 controller. Only works with specific screens - dont know which
...
void gLCD::twoPixels(char Colour){
	...
	//If pixel one foreground or background
	if (pixelOne){
		_SendRG = _ForeRed << 4;
		_SendRG |= _ForeGreen;
		_SendBR = _ForeBlue << 4;
...
void gLCD::SendByte(char Command, unsigned char data){

		char i;
		if (_fast){
			writeRS(1);					//Startup LCD
			writeCS(0);					//LCD_ChipSel 0 start of sequence
			//delayMicroseconds(1);
			
			//Send Command or parameter
			writeSCLK(0);					//Clock 0
			if (Command){
				writeSDATA(1);		//Send a bit
			} else {
				writeSDATA(0);		//Send a bit
			}
			writeSCLK(1);					//Clock 1
			...
			digitalWrite(_CS, 0);					//LCD_ChipSel 0 start of sequence
			...
			delayMicroseconds(1);
			digitalWrite(_CS, 1);					//LCD_ChipSel to 1 at end of sequence
		}
}
...
void gLCD::Init(char Xzero, char Yzero, boolean InvertColour){
		...
		SendByte(_parameter, 0x04);          //Scan direction (left-->right or right-->left)
...
void gLCD::Init(char Xzero, char Yzero, boolean InvertColour, boolean Phillips, boolean xReverse){
		...
		if (xReverse){ //When xReverse is on, then the value of this parameter is ajusted so that x is mirrored from normal.
			SendByte(_parameter, 0x60);      //RGB order, mirror display x or y. 0x68 = RGB, no mirroring
											 //									 0x60 = BGR, no mirroring
											 //									 0xA0 = BGR, mirror X and Y
											 //									 0xA8 = RGB, mirror X and Y
		} else {
			SendByte(_parameter, 0x20);      //RGB order, mirror display x or y. 0x28 = RGB, no mirroring
